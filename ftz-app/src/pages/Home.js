...
import { logout, emailVerification, auth, addTodo, deleteTodo } from "../firebase";
...
const Home = () => {
      ...
      <>
        <h1 className="text-2xl font-bold flex gap-x-4 items-center justify-center">
          {auth.currentUser.photoURL ? (
            <img src={auth.currentUser.photoURL} alt="avatar" className="w-12 h-12 rounded-full " />
          ) : null}
        ...
        <ul ref={parent} className="mt-4 flex flex-col gap-y-2">
          {todos.length > 0 ? (
            ...
            <li className="text-center text-yellow-700">Hiç Todo Eklemediniz!!</li>
          )}
        </ul>
      </>
    );
  ...
  return (
    <>
      <div className="flex flex-col-1 justify-center items-center mt-10">
        <h1 className="text-3xl font-black bg-indigo-500  text-white rounded">
          React Firebase Authentication Project
        </h1>
      </div>
      <div>
        <Link
          className="flex items-center justify-center h-8 rounded px-4 text-sm text-white bg-indigo-500 hover:bg-indigo-800 hover:shadow-lg hover:font-bold m-2"
          to="/register"
        >
          <h1>Kayıt Ol</h1>
        </Link>
